...
def prim(s_node, graph):
    ...
    candidate_edge_lst = adj_edges[s_node]
    heapify(candidate_edge_lst)
    while candidate_edge_lst:
        w, n1, n2 = heappop(candidate_edge_lst)
        if n2 not in conn_nodes:
            conn_nodes.add(n2)
            mst.append((w, n1, n2))
            for edge in adj_edges[n2]:
                if edge[2] not in conn_nodes:
                    heappush(candidate_edge_lst, edge)
    return mst
